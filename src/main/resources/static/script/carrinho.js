...
function mostraCarrinho() {
  ...
  if (carrinho != null) {
    ...
    <div class = "row" style="text-align:left;">
    	<div class  = "col-6">
      		<img src="../img/prodImages/${
            produto.img
          }.jpg"  style = "width:100px;height:100px; object-fit:contain;margin:auto;"alt="${
        produto.nome
      }">
     	 </div>
      	 <div class = "col-6">
      		<h5>${produto.nome} <span id="produto-carrinho-${
        produto.id
      }"class="badge badge-primary badge-pill">${produto.quantidade}</span></h5>
      		<p>R$ ${produto.preco.toFixed(2)}</p>
      		<div class = "row justify-content-around">
      			<button class = "btn  btn-sm btn-light" onclick ="aumentarQuantidade(${
              produto.id
            }),mostraCarrinho()"><i class="fas fa-plus"></i></button>
      			<button class = "btn  btn-sm btn-light" onclick ="diminuirQuantidade(${
              produto.id
            }),mostraCarrinho()"><i class="fas fa-minus"></i></button>
      		</div>
      	</div>
    </div>
  `;
